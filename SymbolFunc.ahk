...
class CnEnPunctSwitcher {
    ...
    static SetupInputHook() {
        ; V 表示不拦截按键，L0 表示不收集文本缓冲区
        this.IH := InputHook("V L0")
        
        ; 关键逻辑：InputHook 默认通知所有按键。
        ; 我们只需要把“配置中的标点键”的通知关掉 (-N)。
        ; 这样，按下标点时由 Hotkey 处理；按下其他任何键（字母/空格/回车）都会触发回调清空状态。
        for vkCode in this.PunctVKMap {
            this.IH.KeyOpt("{vk" Format("{:X}", vkCode) "}", "-N")
        }
        
        ; 只要触发了 OnKeyDown，说明按下的绝对不是标点键，直接清空状态
        ; 使用箭头函数进一步精简路径
